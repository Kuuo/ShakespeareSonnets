!function (a) { 
	a.fn.websnowjq = function (b) { 
		...
		function w() { 
			for (var a = 0; a < n.length; a++) 
				n[a].y < k.canvas.height ? 
			(n[a].y += n[a].speed, n[a].y > k.canvas.height 
				&& (n[a].y = -5), n[a].x += n[a].drift, n[a].x > k.canvas.width 
				&& (n[a].x = 0)) 
			: (n.splice(a, 1), n[n.length] = new s) 
		} 

		var c = this[0], 
		d = c.id, e = a(c).offset().left, f = a(c).offset().top, g = a(c).height(), h = a(c).width(), 
		i = "top:" + f + "px;" + "left:" + e + "px;" + "position:absolute;", 
		j = null, k = null, l = null, m = null, n = [], o = null, 
		p = a.extend({ snowFlakes: 100 }, b); 
		a("body").append("<canvas width='" + h + "' height='" + g + "' id='websnowjqcan" + d + "' style='" + i + "'></canvas>"),
		 q() 
	} 
}(jQuery);